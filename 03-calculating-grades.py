...
homework_scores = final_data.filter(regex=r"^Homework \d\d?$", axis=0)
homework_max_points = final_data.filter(regex=r"^Homework \d\d? -", axis=0)
...
final_data["Homework Score"] = final_data[["Total Homework", "Average Homework"]].max(
    axis=1
)
...
final_data["Ceiling Score"] = numpy.ceil(final_data["Final Score"] * 100)
